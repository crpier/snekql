...
from snekql import mariadb as database
from snekql.mariadb.model import ModelMeta
from union_probe import foreign_key
...
class Account[S=database.Pending](database.Model[S, "Account[database.Fetched]"], metaclass=Meta):
    ...
    required: database.FKCol[Account, int] = foreign_key(lambda: Account.key)
    ...
    nullable_value_default: database.FKCol[Account, int | None] = foreign_key(lambda: Other.key, nullable=True, default=1)
