"""Expected family rejections for the propagated-family prototype."""
...
_ = sqlite.select(SqliteUser).join(
    MariadbUser,  # ty: ignore[invalid-argument-type]
    on=JoinOn[SqliteFamily, SqliteUser[Pending], MariadbUser[Pending]](),
)
_ = mariadb.select(MariadbUser).join(
    SqliteUser,  # ty: ignore[invalid-argument-type]
    on=JoinOn[MariadbFamily, MariadbUser[Pending], SqliteUser[Pending]](),
)
