"""Real MariaDB observations for the nested review file."""
...
async def database() -> AsyncGenerator[native.Database]:
    ...
    async with await native.Database.initialize(instance.config()) as connected:
        await connected.migrate({"001": app.mariadb.scaffold(app.Product)})
        yield connected
...
async def product_insert_returns_precise_complete_contract() -> None:
    ...
    assert_type(product, app.Product)
...
async def product_select_decodes_native_values() -> None:
    ...
    async with connected.transaction() as raw:
        products = await app.mariadb.Transaction(raw).fetch_all(
            app.mariadb.select(app.Product).all()
        )
    assert_type(products, list[app.Product])
    assert_eq(
        (
            products[0].token,
            products[0].payload,
