"""Real MariaDB observations for the input-first review file."""
...
async def database() -> AsyncGenerator[native.Database]:
    ...
    async with await native.Database.initialize(instance.config()) as connected:
        await connected.migrate(
            {"001": app.mariadb.scaffold(app.ProductRow, app.PriceReferenceRow)}
        )
        yield connected
...
async def seeded_database() -> AsyncGenerator[native.Database]:
    async with database() as connected:
        async with connected.transaction() as raw:
            await app.create_product(app.mariadb.Transaction(raw))
        yield connected
...
async def product_insert_returns_precise_complete_contract() -> None:
    ...
    async with connected.transaction() as raw:
        product = await app.create_product(app.mariadb.Transaction(raw))
    assert_type(product, app.ProductRow)
...
async def product_select_decodes_native_values() -> None:
    ...
    async with connected.transaction() as raw:
        products = await app.mariadb.Transaction(raw).fetch_all(
            app.mariadb.select(app.ProductRow).all()
        )
    assert_type(products, list[app.ProductRow])
    assert_eq(
        (
            products[0].token,
            products[0].payload,
...
async def json_expression_retains_nullable_integer_result() -> None:
    connected = await load_fixture(seeded_database())
    async with connected.transaction() as raw:
        answers = await app.product_answers(app.mariadb.Transaction(raw))
    assert_type(answers, list[int | None])
    assert_eq(answers, [42])


@test(mark="slow")
async def decimal_foreign_key_join_uses_native_values() -> None:
    connected = await load_fixture(seeded_database())
    async with connected.transaction() as raw:
        await app.mariadb.Transaction(raw).execute(
            app.mariadb.insert(app.PriceReference(amount=Decimal("12.50")))
        )
    async with connected.transaction() as raw:
        prices = await app.mariadb.Transaction(raw).fetch_all(
            app.mariadb.select(app.ProductRow.price)
            .join(
                app.PriceReferenceRow,
                on=app.PriceReferenceRow.amount.references(app.ProductRow.price),
            )
            .all()
        )
    assert_type(prices, list[Decimal])
    assert_eq(prices, [Decimal("12.50")])
