"""Explicit native bridges for verbs absent from the input-first research adapter.
...
async def native_delete(
    transaction: sqlite.Transaction, row: type[sqlite.Model], *, key: tuple[str, object]
) -> None:
    model = Schema(row).native(row)
