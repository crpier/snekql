...
**Migration**:
A named, hand-authored change to schema or data. Its identity includes its position and exact SQL body. Migrations are the sole schema-creation authority. snekql runs and tracks them but never authors them; the application author owns each change.
_Avoid_: autogenerated migration, schema diff, auto-upgrade

**Migration Declaration**:
The complete ordered Migration chain supplied by one code version. Every declaration extends or preserves one canonical linear history; it is not an independently composable subset.
