...
CHECKER_PANIC_EXIT = 101
"""A checker crash is not evidence that a bad caller was correctly rejected."""
...
async def check_case(case: Case) -> dict[str, object]:
    ...
    header = await AsyncPath(
        root / "typing_probes/model_definitions" / case.template
    ).read_text()
    source = (
        header
        + "\n\ndef probe() -> None:\n"
    ...
    async with TemporaryDirectory(dir=str(root / ".git")) as directory:
        ...
        for negative in (False, True) if case.invalid else (False,):
            rendered = source + (
                "\n".join("    " + line for line in case.invalid.splitlines()) + "\n"
                if negative
                else ""
            )
            ...
            try:
                with fail_after(5 if case.checker_failure else 30):
                    checked = await run_process(arguments, env=environment, check=False)
            except TimeoutError:
                observations.append(
                    {
                        "negative": negative,
                        "conforms": negative and case.checker_failure,
                        "source_sha256": sha256(rendered.encode()).hexdigest(),
                        "timed_out_seconds": 5 if case.checker_failure else 30,
                    }
                )
                continue
            ...
            expected_line = (
                ...
                if case.error_line == -1
                else len(source.splitlines()) + case.error_line
            )
            ...
            if negative and case.checker_failure:
                # Recursive constructor capture can either hang or hit ty's
                # cycle limit. Neither outcome is an ordinary static rejection.
                conforms = checked.returncode == CHECKER_PANIC_EXIT and any(
                    diagnostic.check_name == "panic" for diagnostic in diagnostics
                )
            observations.append(
                {
                    "negative": negative,
    ...
    return {
        "name": case.name,
        "expected_rejection": (
            case.rejects if case.invalid and not case.checker_failure else None
        ),
        "expected_checker_failure": case.checker_failure,
