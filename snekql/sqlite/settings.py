...
SQLite applies them per connection. ``foreign_keys`` backs the FK constraints
emitted in the schema layer (without it they are inert), ``busy_timeout`` keeps
the multi-connection pool from raising spurious "database is locked" errors, and
``encoding`` is a build-time guard that text is stored as UTF-8.
"""
...
async def apply_sqlite_connection_settings(connection: Connection) -> None:
