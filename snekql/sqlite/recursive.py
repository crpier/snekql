...
def recursive_cte[OwnerT: Table[Any], ResultT: BaseModel, RoleT, NonNullableOwnerT](
    ...
    *,
    name: str,
    step: Callable[
        [_Cte[Literal["sqlite"], OwnerT, ResultT, RoleT, NonNullableOwnerT]],
        _NamedSetOperand[Literal["sqlite"], ResultT, _ExecutableQuery],
    ],
) -> _Cte[Literal["sqlite"], OwnerT, ResultT, RoleT, NonNullableOwnerT]:
    """Build an anchor UNION ALL its callback's direct recursive member."""
    return build_recursive_cte(anchor, role, name=name, step=step, backend="sqlite")
