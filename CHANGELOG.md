...
- Replaced the structured-logger requirement with standard-library `logging`. `Database.initialize(...)` and `Database.migrate(...)` no longer accept a `logger=` argument, and `StructuredLogger` is no longer exported from the backend namespaces. snekql now logs through the stdlib `logging` hierarchy under per-module loggers rooted at `snekql` (`snekql.runtime`, `snekql.sqlite.runtime`, …), emitting conventional message-style records (`logger.info("...", arg)`) instead of structlog-style event dicts. An application controls all snekql output from the one `snekql` logger (e.g. `logging.getLogger("snekql").setLevel(logging.WARNING)`); a `NullHandler` is attached at import so snekql stays silent until the app configures logging. To recover machine-readable fields, point a JSON/structured formatter (such as `structlog`'s `ProcessorFormatter`) at the handler receiving `snekql` records.
